...
// until Steven pastes). Originally asserted "objectives" must be in the
// version string, but every subsequent bot PR bumps the version with its
// own keyword (e.g. "budget-intent-guard"). Loosened to "starts with
// YYYY-MM-DD" so it survives future bumps.
const v = (BOT.match(/KFL_BUILD_VERSION\s*=\s*['"]([^'"]+)['"]/) || [])[1];
assert(v && /^\d{4}-\d{2}-\d{2}/.test(v),
  'KFL_BUILD_VERSION starts with YYYY-MM-DD (currently: ' + v + ')');
