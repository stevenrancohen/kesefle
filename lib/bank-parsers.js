...
// Discount, Mizrahi-Tefahot). Returns a normalized array of transactions
// plus a `skipped` array for rows that could not be parsed.
//
// Design notes:
...
//                  We support both.
//       Discount:  same two-column scheme as Hapoalim. Columns are usually
//                  "סכום חיוב" / "סכום זיכוי".
//       Mizrahi:   same two-column scheme. Columns are usually
//                  "סכום חובה" / "סכום זכות".
...
//   parseDiscountCsv(text|buffer) -> { rows: TxRow[], skipped: SkipRow[] }
//   parseMizrahiCsv(text|buffer)  -> { rows: TxRow[], skipped: SkipRow[] }
//   BANK_PARSERS = { hapoalim, leumi, discount, mizrahi }
//
// TxRow shape:
//   {
//     date: 'YYYY-MM-DD',
...
// "when the money moved" and matches the bank's own dashboard. The Mizrahi
// "תאריך הפעולה" variant is added explicitly for clarity (it would also
// match the "תאריך" alias via substring, but explicit is better).
const DATE_ALIASES = ['תאריך ערך', 'תאריך הערך', 'ערך', 'תאריך הפעולה', 'תאריך'];
const PREFERRED_DATE_ALIAS = ['תאריך ערך', 'תאריך הערך', 'ערך']; // fall back to 'תאריך' / 'תאריך הפעולה'

// Description / narration column. "פירוט" (Discount) is distinct from "פרטים"
// (Hapoalim) -- different word, same meaning.
const DESC_ALIASES = ['פרטים', 'פירוט', 'תיאור', 'תיאור הפעולה', 'הסבר', 'שם פעולה'];

// Hapoalim-style two-column scheme. Discount uses "סכום חיוב"/"סכום זיכוי",
// Mizrahi uses "סכום חובה"/"סכום זכות" -- listed explicitly here.
const DEBIT_ALIASES = ['חובה', 'בחובה', 'חיוב', 'סכום חיוב', 'סכום בחובה', 'סכום חובה'];
const CREDIT_ALIASES = ['זכות', 'בזכות', 'זיכוי', 'סכום זיכוי', 'סכום בזכות', 'סכום זכות'];
...
// Mizrahi's "אסמכתא/מספר" matches via substring on "אסמכתא".
const REF_ALIASES = ['אסמכתא', 'מספר אסמכתא', 'אסמכתה', 'אסמכתא/מספר'];
...
function parseCsvWithScheme(input, scheme /* 'hapoalim' | 'leumi' */) {
  ...
  for (let i = headerIdx + 1; i < allCells.length; i++) {
    ...
    if (!date) {
      ...
      // "יתרת סגירה" (closing balance) and "יתרת חשבון" (account balance) use
      // the construct form יתרת -- accept that as well as plain יתרה.
      const isSummary = cells.some(c => /^(יתרה|יתרת|סה[״\\"]כ|סך הכל|סיכום)/.test(String(c).trim()));
...
// Discount uses the same two-column debit/credit scheme as Hapoalim, so we
// reuse the 'hapoalim' fallback behavior (single-amount column would default
// to expense, not income). Header aliases differ -- those are already in the
// shared alias arrays above.
export function parseDiscountCsv(input) {
  return parseCsvWithScheme(input, 'hapoalim');
}

// Mizrahi-Tefahot also uses the two-column scheme. Same reasoning as Discount.
export function parseMizrahiCsv(input) {
  return parseCsvWithScheme(input, 'hapoalim');
}
