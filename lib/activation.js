...
// 1h result cache (KV quota guard, audit 2026-06-09): the cohort scan costs
// ~1.5K KV commands per run (scan + per-user GETs + 2 health GETs each). The
// admin card fetches it on every page load and the daily digest daily -- on
// the free Upstash tier (500K cmds/mo) that trajectory hits the cap mid-month
// and stalls the BOT. Cached result serves for 1h; pass {fresh:true} to skip.
...
async function cacheSet(key, value) {
  try {
    await fetch(`${KV_URL}/set/${encodeURIComponent(key)}?EX=${CACHE_TTL_SEC}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${KV_TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify(value),
    });
  } catch (_e) { /* best-effort -- never block the summary */ }
}

export async function computeActivationCohort(days = 30, opts = {}) {
  if (!KV_URL || !KV_TOKEN) return { ok: false, error: 'kv_unavailable' };
  const cacheKey = `activation:summary:${days}`;
  if (!opts.fresh) {
