...
const TENANT_SUMMARY_CATS = ['הכנסות', 'אוכל', 'תחבורה', 'הוצאות קבועות', 'הוצאות זמניות', 'קניות', 'בריאות', 'עסק', 'שירותים', 'בידור', 'חינוך', 'ילדים', 'ממשלה ומיסים', 'פיננסים', 'אחר'];
...
export function buildTenantSheetSpec(name) {
  ...
  sum.push({ values: [_sw_bold(_sw_str('קטגוריה')), _sw_bold(_sw_str('סכום'))] });                                  // row 6
  TENANT_SUMMARY_CATS.forEach(function (cat) {
    sum.push({ values: [_sw_str(cat), _sw_formula('=SUMIF(תנועות!E:E,"' + cat + '",תנועות!B:B)')] });
  });
  return {
