...
//
// 2026-06-03 sign-flip fix: each row now also filters col H (סטטוס) —
// income rows by H=FALSE, expense rows by H=TRUE. `isIncomeRow` selects
// which. Without it the wildcard col-E match crossed the income/expense
// boundary: e.g. the personal income subcategory "שונות (הכנסות)" CONTAINS
// "שונות", so the expense row "שונות" (criterion "*שונות*") double-counted
// that income AS a cost, understating net/savings. The repair builder
// (bot/personal_sheet_fix.gs) already segregates income (H=FALSE) from
// expense (H=TRUE); the generator now matches. buildExpenseRow writes col H
// as a real boolean (`!isIncome`), so the bare boolean literal TRUE/FALSE is
// the correct SUMIFS criterion for the rows this template creates.
function _personalCategoryRow(rowNum, label, isIncomeRow) {
  const statusCrit = isIncomeRow ? 'FALSE' : 'TRUE';
  const cells = [];
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    cells.push(_sw_currencyFmt(_sw_formula(
      `=IFERROR(SUMIFS('${TX_TAB}'!C:C, '${TX_TAB}'!B:B, $B$2&"-${mm}", '${TX_TAB}'!E:E, "*"&$A${rowNum}&"*", '${TX_TAB}'!H:H, ${statusCrit}), 0)`
    )));
  }
...
function _buildPersonalDashboardTab(defaultYear) {
  ...
  // R5-8: income categories (isIncomeRow=true -> SUMIFS filters col H=FALSE).
  PERSONAL_INCOME_ROWS.forEach(function (label, i) {
    rows.push(_personalCategoryRow(5 + i, label, true));
  });
...
function _buildCompanyDashboardTab(defaultYear) {
  ...
  //
  // 2026-06-03 sign-flip fix: every expense SUMIFS now also filters col H
  // (סטטוס) = TRUE (expense). Without it, the row summed by col D="עסק" +
  // col E=<criterion> ALONE, so any business *income* whose subcategory text
  // happened to match an expense wildcard (e.g. a future income label
  // containing "שיווק"/"משלוח") would be counted as a cost — understating net
  // profit. The repair/recompute builder (bot/personal_sheet_fix.gs,
  // _psf_buildFormula_v2_) already applies H=TRUE to every expense bucket and
  // H=FALSE to revenue; the generator now matches that contract so a fresh
  // sheet and a repaired sheet produce identical totals. buildExpenseRow
  // writes col H as a real boolean (`!isIncome`), so the bare boolean literal
  // TRUE is the correct SUMIFS criterion for the rows this template creates.
...
function _buildExtendedDashboardTab(defaultYear) {
  ...
  //
  // 2026-06-03 sign-flip fix: also filter col H (סטטוס) — income rows by
  // H=FALSE, expense rows by H=TRUE (`isIncomeRow` selects). This tab's
  // INCOME_GROUPS and EXPENSE_GROUPS use the SAME 88 subcategory labels
  // (Pa'amonim convention: e.g. "מזון", "חופשות", "ביטוח לאומי" exist on
  // both sides). Without the col-H filter, every income row's exact-match
  // SUMIFS on col E ALSO swept the identically-named expense rows (and vice
  // versa), so the income total and expense total each contained the other
  // side's money — corrupting net/savings. Matches the repair builder
  // (bot/personal_sheet_fix.gs) income/expense segregation. buildExpenseRow
  // writes col H as a boolean, so TRUE/FALSE is the correct SUMIFS criterion.
  function emitSubcatRow(rowNum, label, isIncomeRow) {
    const statusCrit = isIncomeRow ? 'FALSE' : 'TRUE';
    ...
    for (let m = 1; m <= 12; m++) {
      const mm = String(m).padStart(2, '0');
      cells.push(_sw_currencyFmt(_sw_formula(
        `=IFERROR(SUMIFS('${TX_TAB}'!C:C, '${TX_TAB}'!B:B, $B$1&"-${mm}", '${TX_TAB}'!E:E, $A${rowNum}, '${TX_TAB}'!H:H, ${statusCrit}), 0)`
      )));
    }
    return _sw_row(cells);
  }
  ...
  INCOME_GROUPS.forEach(function (group) {
    ...
    group.items.forEach(function (item) {
      currentRow += 1;
      // isIncomeRow=true -> SUMIFS filters col H=FALSE (income only).
      rows.push(emitSubcatRow(currentRow, item, true));
    });
    // Section total
    currentRow += 1;
  ...
  EXPENSE_GROUPS.forEach(function (group) {
    ...
    group.items.forEach(function (item) {
      currentRow += 1;
      // expense row -> emitSubcatRow defaults to col H=TRUE (expense only).
...
export function sanitizeCell(v) {
  ...
  // Defense-in-depth (audit 2026-06-15): explicitly neutralize data-exfiltration
  // formulas regardless of leading char, so the guard survives a future drift away
  // from valueInputOption=RAW (these can pull/leak data and embed remote calls).
  if (/=\s*(IMPORTRANGE|IMPORTXML|IMPORTDATA|IMPORTFEED|IMPORTHTML|HYPERLINK|IMAGE)\s*\(/i.test(cleaned)) {
    return "'" + cleaned;
  }
...
export function normalizeSubcategoryForDashboard(subcategory, category, isIncome) {
  ...
  // 0. PERSONAL INCOME GUARD (disappearing-income fix 2026-06-27): a row the
  // sign-resolver marked as income MUST land on a personal income row. Otherwise
  // col H = income (FALSE) paired with an EXPENSE col-E row label means the SUMIFS
  // pick it up in NEITHER total (income rows filter col H = income, expense rows
  // filter col H = expense) -> the money disappears from the dashboard. This bites
  // NL-detected income (e.g. "גבייה"/"לקוחה שילמה") that classifies to a default
  // subcategory. Business income (cat === 'עסק') is dashboarded on מאזן חברה
  // separately, so it is excluded here.
  if (isIncome && cat !== 'עסק') {
    if (Object.prototype.hasOwnProperty.call(SUB_TO_DASHBOARD_ROW, raw) &&
        PERSONAL_INCOME_ROWS.indexOf(SUB_TO_DASHBOARD_ROW[raw]) >= 0) {
      return SUB_TO_DASHBOARD_ROW[raw];
    }
    for (let i = 0; i < PERSONAL_INCOME_ROWS.length; i++) {
      if (raw.indexOf(PERSONAL_INCOME_ROWS[i]) >= 0) return PERSONAL_INCOME_ROWS[i];
    }
    return 'שונות (הכנסות)';
  }
...
export function buildExpenseRow({ amount, category, subcategory, rawText, date, isIncome, vatDeductible }) {
  ...
  //
  // TZ-STABLE CALENDAR MONTH (2026-06-03): col B (חודש, "YYYY-MM") is the key
  // every dashboard SUMIFS buckets by, so it MUST reflect the intended
  // CALENDAR month. When a caller passes a date-only string ("2026-06-01") or
  // a UTC-midnight calendar date ("2026-06-01T00:00:00Z") — which is exactly
  // what the recurring engine (api/recurring.js, date = dateStr) and the CSV
  // importer (date = r.date + 'T00:00:00Z') do — `new Date(s).getMonth()`
  // reads the month in the RUNTIME's local timezone. Under a negative-offset
  // TZ (e.g. a non-UTC CI box or region) UTC-midnight rolls back to the
  // previous day, so "2026-06-01" silently filed under "2026-05" (and a Jan-1
  // backfill under the PREVIOUS YEAR). Vercel runs UTC so prod is unaffected
  // today, but the bug is latent and defeats the careful todayIsrael() calendar
  // pinning upstream. So: if the input is a bare calendar date, take YYYY-MM
  // straight from the literal digits — no Date round-trip, no TZ ambiguity.
  let d, dateISO, yyyy, mm;
  const calMatch = (typeof date === 'string')
    ? date.match(/^(\d{4})-(\d{2})-(\d{2})(?:[T ]00:00(?::00)?(?:\.000)?Z?)?$/)
    : null;
  if (calMatch) {
    // Bare calendar date (optionally with an explicit UTC-midnight time). Pin
    // the row to that exact day at UTC NOON so neither col B (month) nor the
    // col-A display day can drift across a timezone boundary in any reader.
    yyyy = calMatch[1];
    mm = calMatch[2];
    dateISO = `${calMatch[1]}-${calMatch[2]}-${calMatch[3]}T12:00:00.000Z`;
  } else {
    // A full timestamp, or no date (live write -> now). Keep the original
    // behaviour exactly: derive everything from the parsed Date.
  ...
  const dashSub = (typeof normalizeSubcategoryForDashboard === 'function')
    ? normalizeSubcategoryForDashboard(subcategory || '', category || '', !!isIncome)
    : (subcategory || '');
  ...
  return [
    ...
    // identical rows. Dashboards DO filter by col H (income rows feed the
    // income totals, expense rows the expense SUMIFS) — keep this sign correct.
    !isIncome,
