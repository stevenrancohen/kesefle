...
export async function appendRowToTab({ refreshTokenEnvelope, refreshToken, userSub, spreadsheetId, tabName, row }) {
  ...
  if (resp.status === 401) {
    try {
      accessToken = await exchangeRefreshForAccess(actualRefresh, userSub);
...
export async function exchangeRefreshForAccess(refreshToken, userSub) {
  const { exchangeRefreshForAccess: exchange } = await import('./oauth.js');
  const { accessToken } = await exchange({ refreshToken, userSub });
  return accessToken;
}
