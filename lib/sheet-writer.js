...
export function buildTenantSheetSpec(name) {
  ...
  return {
    // Google Sheets uses the LEGACY code 'iw_IL' for Hebrew/Israel (not 'he_IL',
    // which the API rejects as "Unsupported locale"). createUserSheetWithToken
    // also retries without locale if any value is rejected, so creation never
    // fails over a locale string.
...
export async function createUserSheetWithToken(accessToken, name) {
  ...
  let res = await attempt(buildTenantSheetSpec(name));
  // Some Google accounts/regions reject locale/timeZone in properties
  // ("Unsupported locale", "Invalid properties"). Retry once WITHOUT them — the
  // tab data + SUMIFS formulas are locale-independent (the API parses formulas
  // in en_US regardless), so dropping locale never affects correctness.
  if (!res.ok && /locale|invalid propert|timezone|time zone/i.test(JSON.stringify(res.j?.error || ''))) {
    const spec2 = buildTenantSheetSpec(name);
    if (spec2.properties) { delete spec2.properties.locale; delete spec2.properties.timeZone; }
