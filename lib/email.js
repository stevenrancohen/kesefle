...
function readTemplateOnce(name) {
  ...
  try {
    ...
    // Fallback: strip a "_dayN" / "_v2" / "_X" suffix and retry the lookup so
    // the lifecycle cron's per-day dedup keys (payment-failed_day3, day7) can
    // route to a single base template file. The guard key in cron lifecycle
    // still differs per "name", so we don't re-send the same email twice.
    const base = name.replace(/_(day\d+|v\d+|[a-z]\d+)$/i, '');
    if (base !== name) {
      const baseFile = base.endsWith('.html') ? base : `${base}.html`;
      try {
        const html = fs.readFileSync(path.join(TEMPLATES_DIR, baseFile), 'utf8');
        TEMPLATE_CACHE.set(name, html);
        return html;
      } catch (_eFallback) {}
    }
    log.warn('email.template_missing', { name });
    return null;
  }
}
