...
import { activatePremium } from './crypto-webhook.js';
...
export default async function handler(req, res) {
  ...
  try {
    const sub = await activatePremium(testUserId, `test_charge_${Date.now()}`);
    return res.status(200).json({ ok: true, userId: testUserId, expiresAt: sub.expiresAt });
  } catch (e) {
