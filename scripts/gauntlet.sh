...
# ============================================================================
printf '\n%s\n' "${B}[1/8] QA gate -- node tests/full_qa.js${Z}"
hr
...
# ============================================================================
printf '\n%s\n' "${B}[2/8] Test suites -- every tests/* + bot/test_* (auto-discovered)${Z}"
hr
g2_pass=0; g2_fail=0
...
# ============================================================================
printf '\n%s\n' "${B}[3/8] JS syntax -- node --check on all *.js + bot *.gs${Z}"
hr
...
# ============================================================================
printf '\n%s\n' "${B}[4/8] HTML -- inline <script> parse + JSON-LD validate${Z}"
hr
...
# ============================================================================
printf '\n%s\n' "${B}[5/8] Sitemap -- sitemap.xml structure + <loc> origins${Z}"
hr
...
# ============================================================================
printf '\n%s\n' "${B}[6/8] Secret scan -- committed html/js/gs/md${Z}"
hr
...
# ============================================================================
printf '\n%s\n' "${B}[7/8] wa-sim corpus -- income-sign + amount regression ratchet${Z}"
hr
...
# ============================================================================
# GROUP 8 -- Localized (i18n) pages under ar/ ru/ fr/ it/ en/
#   Correct <html lang/dir>, reciprocal hreflang, language switcher, one <h1>,
#   inline scripts parse, no bidi controls. No-op until localized pages exist.
# ============================================================================
printf '\n%s\n' "${B}[8/8] i18n -- localized /<lang>/ pages structure + hreflang${Z}"
hr
if run_counted_helper node "$HELP/check-i18n.js" "$ROOT"; then
  record_group "8. i18n localized pages" "$RG_PASS" "$RG_FAIL"
else
  record_group "8. i18n localized pages" 0 1
fi
