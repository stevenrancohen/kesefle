...
| 4 | קולקציות                    | src_collection_business  | עסק                    | 💼 קולקציות             | business           | business_expense    | 2026-05-31: Steven CONFIRMED קולקציות is his SRC BUSINESS (SRC Collection), NOT a personal hobby. Rerouted category תחביבים → `עסק` (company dashboard). Bot tags new writes as `עסק`/`קולקציות`; the dedicated dashboard row lives on the future 'עסק 2' SRC tab (separate work), so no `_BIZ_DASH_SUBS` mapping yet. Custom — NOT in Pa'amonim. `is_custom=TRUE`. |
